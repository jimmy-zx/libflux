...
__attribute__((no_instrument_function, noinline)) void __sanitizer_cov_trace_pc(
    void) {
  uintptr_t pc = (uintptr_t)__builtin_return_address(0) - ELF_ET_DYN_BASE;
#ifdef PATH_COVERAGE
  uintptr_t idx = (pc ^ (__trace_pc_last_pc >> 1)) & (kNumCounters - 1);
#else
  uintptr_t idx = pc & (kNumCounters - 1);
#endif
  if (__trace_pc_config.verbose) {
...
__trace_pc_init(void) {
  ...
  __trace_pc_config.verbose = (getenv("FLUXCOV_VERBOSE") != NULL);
  ...
  if (__trace_pc_config.filter != NULL) {
    ...
    if (strcmp(__trace_pc_config.filter, base) != 0) {
      ...
      __trace_pc_config.verbose = false;
    }
    free(exec);
  }
  ...
  if (getenv("FLUXCOV_HELP") != NULL) {
    ...
    printf(
        ...
        "cleared (only valid in SHM mode)\n"
        "\tFLUXCOV_VERBOSE=[any=%d]: if enabled, prints the pc when the hook "
        "is called\n"
        ...
        (int)__trace_pc_config.verbose, __trace_pc_config.filter,
        (long long)getpid(), exec);
    free(exec);
  }
